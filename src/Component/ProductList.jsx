...
function ProductList({ products, onAdd, onEdit, onRemove, onBillComplete }) {
  ...
  const calculateTotal = () => {
    return filteredProducts.reduce(
      (sum, item) => sum + (item.price * item.quantity * (1 + item.gst / 100) * (1 - item.discount / 100)),
      0
    );
  };
  ...
  <form onSubmit={handleSubmit}>
    <div className="grid grid-cols-8 gap-2">
      {[
        ...
        { label: 'Price', name: 'price', value: product.price, type: 'number', min: 0, step: 0.01 },
        ...
        <div key={name} className={`col-span-${rest.colSpan || 1}`}>
          ...
          <input
            ...
            {...rest}
            className={`w-full px-2 py-1 text-sm border ${rest.readOnly ? 'bg-gray-100' : 'bg-white'} border-gray-300 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500`}
            ...
            filteredProducts.map((item, index) => (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 text-right border-b border-gray-200 font-bold">
                  ₹{(item.price * item.quantity * (1 + item.gst / 100) * (1 - item.discount / 100)).toFixed(2)}
                </td>
