...
const Sales = () => {
  ...
  const [selectedCustomerPeriod, setSelectedCustomerPeriod] = useState('all'); // New state for customer dropdown selection
  const [filteredBills, setFilteredBills] = useState([]); // New state for filtered bills
  const [totalAmounts, setTotalAmounts] = useState({ // New state for total transaction amounts
    allTime: 0,
    day: 0,
    week: 0,
    month: 0,
  ...
  const calculateTotalAmounts = (billsData) => {
    const today = new Date();
    ...
    <div className="p-6 bg-gray-50 min-h-screen font-inter">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold text-gray-900">Transaction Overview</h2>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              ...
              <div className="bg-white rounded-lg shadow-sm p-5 border border-yellow-100 flex flex-col justify-between">
                <div>
                  <div className="text-sm font-medium text-yellow-600">Total Revenue (All Time)</div>
                  <div className="text-3xl font-bold text-gray-900 mt-1">₹ {totalAmounts.allTime.toFixed(2)}</div>
                </div>
                <p className="text-gray-500 text-xs mt-2">Cumulative earnings</p>
              </div>

              {/* Dynamic Revenue Card */}
              <div className="bg-white rounded-lg shadow-sm p-5 border border-purple-100 flex flex-col justify-between">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <select
                    ...
                    className="block w-auto px-2 py-0.5 text-xs border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm"
                  >
                ...
                <div className="text-3xl font-bold text-gray-900">
                  ₹ {totalAmounts[selectedProductPeriod]?.toFixed(2) || '0.00'}
                </div>
                <p className="text-gray-500 text-xs mt-2">Earnings for selected period</p>
              </div>
            </div>
